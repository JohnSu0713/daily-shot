...
export default function Home(){
 ...
 const [locale,setLocale]=useState<Locale>("zh"); const [theme,setTheme]=useState<Theme>("light");
 ...
 useEffect(()=>{const l=localStorage.getItem("daily-shot:locale");if(l==="en"||l==="zh")setLocale(l);else{setLocale("zh");localStorage.setItem("daily-shot:locale","zh");document.documentElement.lang="zh-Hant"}const saved=localStorage.getItem("daily-shot:theme");const initial=saved==="dark"||saved==="light"?saved:(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");setTheme(initial);document.documentElement.dataset.theme=initial;const items=readJournal();setJournal(items);setNote(items.find(e=>e.date===today.dateKey)?.note||"")},[today.dateKey]);
 ...
 return <main className="shell">
  ...
  {tab==="today"?<><section className="photo-stage">{!imageFailed?<><>{!imageLoaded&&<div className="image-skeleton"><span>{t.loading}</span></div>}</><img className={`photo ${imageLoaded?"loaded":""}`} src={today.artwork.imageUrls[imageIndex]} alt={today.artwork.title} onLoad={()=>setImageLoaded(true)} onError={imageError}/></>:<div className="image-error"><span className="brand-mark large"><i/></span><h2>{t.imageBreak}</h2><p>{t.imageBreakBody}</p><a href={today.artwork.sourceUrl} target="_blank">{t.openSource}</a></div>}{timerRunning&&<div className="timer-overlay"><span>{seconds}</span><small>{t.justLook}</small></div>}</section>
   ...
   {revealed&&<section className="lesson"><div className="lesson-grid"><div><div className="eyebrow">{t.concept}</div><h2>{lesson.concept}</h2></div><div className="lesson-copy"><p>{lesson.intro}</p><blockquote>{lesson.takeaway}</blockquote></div></div><div className="practice-card"><div className="practice-number">01</div><div><div className="eyebrow">{t.makeFrame}</div><h3>{t.tryToday}</h3><p>{lesson.practice}</p></div></div></section>}
